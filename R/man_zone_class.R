...
# load in pbp data
weeks <- aggregate_week_files() %>% mutate(event = replace(.data$event, .data$event == 'None', NA)) %>%
  group_by(.data$playId, .data$gameId, .data$nflId) %>%
  slice(-n()) %>%
  fill(.data$event, .direction = 'down') %>%
  filter(.data$event == 'ball_snap') %>%
  ungroup() %>%
  mutate(playDirection = as.integer(.data$playDirection == 'left'),
  ...
  ungroup() %>%
  mutate(dir_ratio = .data$off_dist / sqrt((.data$opponent_x - .data$teammate_x)**2 + (.data$opponent_y - .data$teammate_y)**2),
...
grouped_weeks <- weeks %>%
  ...
  summarise(var_x = var(.data$x, na.rm = T),
