...
The pre-snap model, which by design only considers the players on the field for the offense, performs relatively well for the lack of information with an AUC of $0.59$ and is well-calibrated on a held-out dataset. The pre-throw model performs much better given the extra information, with an $89\%$ recall, $94\%$ precision, and $0.94$ AUC. Calibration plots of the models on held-out data are below.
...
Having produced these four models that gave us estimates of a defender's influence on a given play, we can accumulate these results over an entire season to produce an estimate of that individual skill. As there isn't a straight-forward way to measure the relative value of these skills, we chose to combine the individual skill percentiles for each defender as a measure of their overall skill. With that as the estimate, these are our top 15 pass defending defensive backs in 2018:
...
A full leaderboard of these can be found [on our shiny app](https://bdb-2021.herokuapp.com/). To help display these results and a few other metrics (such as how difficult the DBs assignment was based on pre-snap target probability and actual separation from reciever at throw time) we developed player cards for each qualifying defender. For example, this is our card for #1 defender Richard Sherman:

```{r, echo=FALSE, out.width="75%",,fig.show='hold',fig.align='center'}
knitr::include_graphics(c( "../inst/plots/sherman_card.png"))
```

These cards can also be found on our shiny app.

## Further Work

There are obviously a number of things that we didn't consider or that would be interesting extensions of this project. Two obvious ones are interceptions added and fumbles added, as those are hugely impactful football plays. We also only considered raw changes in aggregating the player stats (i.e. targets prevented and drops added), but using EPA added instead would certainly be a better metric. It would also be interesting to test how similar our metrics are between seasons to confirm that what we're measuring is a stable skill of the defender.
...
There is extensive documentation for our code on our [pkgdown site](hjmbigdatabowl.github.io/bdb2021/reference).
